...
For updates, the server will first get the list of neems for the neemhub. Next it will check the repository of each of these neems for a README. If one can be found, it will be fetched and stored locally. The repositories of each neem is then checked for the time of the last update. Neems are then sorted by this timestamp, in order to select the six most recent neems to display on the homepage. The two neems featured at the top of the homepage are identified by their neem-id. Meanwhile information about the neem and location of the related files will be stored in a python dictionary. The dictionary divides neems into three categories:

- **featured neems**:  the two neems that are featured on top of the homepage
- **recent neems**: the six most recently updated neems
- **all neems**: includes the neems from the mentioned categories and all the others listed in the neemhub
...
Lastly, the content-settings page is where the url for the publications `bibtex` and `papers.zip` need to be set. At least the former needs to be provided, otherwise the publications files cannot be updated.
...
- upon start-up, default content is loaded (=faster start-up); no updates for neem-overview and publications are fetched
